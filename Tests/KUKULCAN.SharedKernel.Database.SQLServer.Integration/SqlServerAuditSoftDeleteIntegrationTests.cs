...
[TestFixture]
[NonParallelizable]
public sealed class SqlServerAuditSoftDeleteIntegrationTests
{
    ...
    {
        var first = new SqlServerIntegrationEntity { TenantId = _tenantId, Name = "A" };
        var second = new SqlServerIntegrationEntity { TenantId = _tenantId, Name = "B" };
    ...
    public async Task AuditInterceptor_ShouldUpdateOnlyModifiedEntityTimestamp()
    {
        ...
        first.Name = "A2";
        await _context.SaveChangesAsync();
        Assert.That(first.ModifiedOn, Is.EqualTo(SqlServerIntegrationConstants.FixedNow));
    ...
    {
        var entity = new SqlServerIntegrationEntity { TenantId = _tenantId, Name = "Delete" };
        _context.Entities.Add(entity);
        await _context.SaveChangesAsync();
        _context.Entities.Remove(entity);
        await _context.SaveChangesAsync();
        var persisted = await _context.Entities.IgnoreQueryFilters().SingleAsync(x => x.Id == entity.Id);
        Assert.That(persisted.ModifiedOn, Is.EqualTo(SqlServerIntegrationConstants.FixedNow));
        Assert.That(persisted.DeletedOn, Is.EqualTo(SqlServerIntegrationConstants.FixedNow));
    }
    ...
    {
        var entity = new SqlServerIntegrationEntity { TenantId = _tenantId, Name = "Excluded" };
        _context.Entities.Add(entity);
        await _context.SaveChangesAsync();
        _context.Entities.Remove(entity);
        await _context.SaveChangesAsync();
    ...
    {
        await using var context = await SqlServerIntegrationContextFactory.CreateAsync(_tenantId);
        var entity = new SqlServerDomainEventEntity { TenantId = _tenantId, Name = "Physical" };
