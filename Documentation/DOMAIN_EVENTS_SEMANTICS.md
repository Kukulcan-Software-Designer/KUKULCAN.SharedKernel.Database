...
```text
SaveChanges
  ├─ Event A → dispatch succeeds → acknowledged
  └─ Event B → dispatch fails     → remains pending

Retry SaveChanges
  └─ Event B → dispatch succeeds → acknowledged

Aggregate events are cleared after all pending events have been successfully dispatched.
```
